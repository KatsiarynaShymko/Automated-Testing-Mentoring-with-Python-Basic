...
class ECommerceApp:
    def __init__(self):
        self.users = {}  # Changed to a dictionary to store usernames and passwords
    ...
    def add_to_cart(self, username, product, quantity):
        ...
        if product in self.cart:
            self.cart[product] += quantity  # Update quantity if product is already in cart
        else:
            self.cart[product] = quantity  # Add product to cart
        return True

    def checkout(self):
        total = sum(self.products[product] * quantity for product, quantity in self.cart.items())
